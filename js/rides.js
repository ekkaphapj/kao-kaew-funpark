// ============================================================
// rides.js — compact park: ferris wheel, carousel, drop tower,
// swing ride, and a ghost coaster circling ABOVE the perimeter
// ============================================================
"use strict";
...
function buildRides(scene) {
  ...
  // =========================================================
  // FERRIS WHEEL  (x=82, z=26)  radius 20
  // =========================================================
  {
    const cx = 82, cz = 26, hubY = 24, R = 20;
    ...
    fwSign.position.set(cx, 3.2, cz - 6.5); // readable from the east street (south)
  ...
  // =========================================================
  // CAROUSEL  (x=30, z=-34) — south side of the east street
  // =========================================================
  {
    const cx = 30, cz = -34, R = 6.5;
    ...
    fallen.position.set(cx + R + 3, 0.35, cz + 2);
    ...
    caSign.position.set(cx, 5.4, cz + R + 1.6); // readable from the street (north)
    caSign.rotation.y = Math.PI;
  ...
  // =========================================================
  // PERIMETER GHOST COASTER — an ellipse of track flying above
  // the park wall, weaving over the whole park
  // =========================================================
  {
    const RX = 100, RZ = 104;
    const heights = [10, 8, 12, 16, 20, 16, 11, 8, 7, 9, 13, 16, 18, 14, 9, 8];
    const raw = heights.map((h, i) => {
      const a = (i / heights.length) * Math.PI * 2;
      return new BABYLON.Vector3(Math.cos(a) * RX, h + 1.5, Math.sin(a) * RZ);
    });
    const curve = BABYLON.Curve3.CreateCatmullRomSpline(raw, 12, true);
    ...
    // sign at the west street end
    const rcSign = BABYLON.MeshBuilder.CreatePlane("rcSign", { width: 10, height: 1.6 }, scene);
    rcSign.position.set(-93, 5.5, -10);
    rcSign.rotation.y = Math.PI / 2; // readable from the east (street side)
    ...
    for (const pz of [-14, -6]) {
      const post = BABYLON.MeshBuilder.CreateCylinder("rcSignPost", { height: 6, diameter: 0.3 }, scene);
      post.position.set(-93.5, 3, pz);
      post.material = tieMat;
  ...
  // =========================================================
  // DROP TOWER  "หอสูงสยองขวัญ"  (x=95, z=-10)
  // =========================================================
  {
    const tx = 95, tz = -10, H = 40;
    ...
    dtSign.position.set(tx - 5, 2.4, tz);
    dtSign.rotation.y = Math.PI / 2; // readable from the street (west)
  ...
  // =========================================================
  // SWING RIDE (chair-o-planes)  (x=-20, z=38)
  // =========================================================
  {
    const sx = -20, sz = 38;
    ...
    for (let i = 0; i < NCH; i++) {
      ...
      chain.parent = pivot;
      if (i === 3) continue; // one chair missing
